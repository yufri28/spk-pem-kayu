...
class Kriteria
{
    ...
    public function tambahKriteria($dataKriteria)
    {
        if ($this->count_kriterial() < 5) {
            ...
            if (mysqli_num_rows($cek) > 0) {
                ...
                $stmtInsert = $this->db->prepare("INSERT INTO kriteria(id_kriteria,nama_kriteria,keterangan) VALUES (?,?,?)");
                $stmtInsert->bind_param("sss", $dataKriteria['id_kriteria'], $dataKriteria['nama_kriteria'], $dataKriteria['keterangan']);
                $stmtInsert->execute();
    ...
    {
        $stmtUpdate = $this->db->prepare("UPDATE kriteria SET nama_kriteria=?, keterangan=? WHERE id_kriteria=?");
        $stmtUpdate->bind_param("sss", $dataKriteria['nama_kriteria'], $dataKriteria['keterangan'], $dataKriteria['id_kriteria']);
        $stmtUpdate->execute();
